...
class BrandList extends Module
{
     ...
     * @var string
     */
    protected $strTemplate = 'mod_iso_brandlist';
    ...
    public function generate()
    {
        if (TL_MODE == 'BE') {
            ...
            $objTemplate->wildcard = '### ISOTOPE ECOMMERCE: BRAND LIST &#169; 360Fusion 2014 ###';
    ...
    protected function compile()
    {
        ...
        $arrCacheIds = null;
				...
				$this->brandname = \Input::get('name');
				$brandData = Brand::getBrandData($this->brandname);
        ...
        $this->Template->brandName = $brandData->name;
				$this->Template->brandDescription = $brandData->description;
				$arrImages = deserialize($brandData->image);
    	  	foreach ($arrImages as $image) { 
    	  		$strImage = 'isotope/' . strtolower(substr($image['src'], 0, 1)) . '/' . $image['src']; 
    	  		$size = @getimagesize(TL_ROOT . '/' . $strImage);
    	  	}       
    	  $this->Template->brandSrc = $strImage;
    	  $this->Template->brandWidth = $size[0];
    	  $this->Template->brandHeight = $size[1];
    	  
				
        // Try to load the products from cache
        if ($this->blnCacheProducts && ($objCache = BrandCache::findForPageAndBrandAndModule($intPage, $this->id, $this->brandID)) !== null) {
        ...
        if (!is_array($arrProducts)) {
            ...
            if ($this->blnCacheProducts) {
                ...
                // Load products
                $arrProducts = $this->findBrandProducts($arrCacheIds);
                ...
                if (ProductCache::isWritable()) {
                    ...
                    BrandCache::deleteForPageAndBrandAndModuleOrExpired($intPage, $this->id, $this->brandID);

                    $objCache = BrandCache::createForPageAndBrandAndModule($intPage, $this->id, $this->brandID);
                ...
                $arrProducts = $this->findBrandProducts();
            }
     ...
     * @return  array
     */
    protected function findBrandProducts($arrCacheIds = null)
    {
    	
    		global $objPage;

    //		echo '$this->brandID:'.$this->brandID;
    		
         $arrColumns    = array();
         $arrCategories = array($objPage->id);
         $arrBrands = array($this->brandID);
        ...
        if (!is_array($arrValues)) {
         ...
         $arrColumns[] = "tl_iso_product.brand IN (" . implode(',', $arrBrands) . ")";
